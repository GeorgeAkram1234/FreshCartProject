...
import { getWishlist, addProductToWishlist, removeProductFromWishlist, isProductInWishlistArray } from '../../wishlistService';
...
export default function Product({ product, index }) {
    ...
    // PERFORMANCE: Use shared React Query cache for wishlist to eliminate N+1 API calls
    // deduplicating requests across all Product components on the page.
    const { data: wishlist = [] } = useQuery({
        queryKey: ['wishlist', userToken],
        queryFn: () => getWishlist(userToken),
        enabled: !!userToken,
        staleTime: 600000, // 10 minutes cache to prevent frequent refetches
    });

    const isInWishlist = isProductInWishlistArray(wishlist, product._id);

    // Mutation for adding/removing from wishlist to keep UI in sync via cache invalidation
    const wishlistMutation = useMutation({
        ...
        onSuccess: () => {
            // Invalidate and refetch wishlist to update all components
            ...
            <div key={index} className="max-w-2xl mx-auto">
                <div className="bg-white shadow-md rounded-lg max-w-sm dark:bg-gray-800 dark:border-gray-700 hover:shadow-2xl transition-all duration-500">
                    <Link to={"/productDetails/" + product._id}>
                        {/* PERFORMANCE: Added loading="lazy" to improve initial page load performance */}
