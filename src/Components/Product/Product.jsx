...
export default function Product({ product, index }) {
    ...
    // Efficiently check if the product is in the wishlist using the global Set
    const isInWishlist = wishlistIds.has(product._id);

    // Handle wishlist toggle (add or remove)
            ...
            <div className="bg-white shadow-md rounded-lg max-w-sm dark:bg-gray-800 dark:border-gray-700 hover:shadow-2xl transition-all duration-500">
                <Link to={"/productDetails/" + product._id}>
                    <img
                        ...
                        alt={product.title}
                        loading="lazy" // Performance optimization: lazy load images below the fold
                    />
                </Link>
