...
export default function Product({ product, index }) {
     ...
     * Using React Query for the wishlist deduplicates the API request across all Product instances.
     * Instead of N requests (one per product) to fetch the wishlist on page load, only 1 request is made.
     */
    const { data: wishlistData } = useQuery({
        queryKey: ['wishlist', userToken],
        queryFn: () => getWishlist(userToken),
        enabled: !!userToken,
        staleTime: 600000, // Cache for 10 minutes to avoid redundant fetches
    });

    const wishlist = wishlistData?.data || [];
    const isInWishlist = isProductInWishlistArray(wishlist, product._id);

    // useMutation handles the API call and provides invalidation to keep the UI in sync
    const wishlistMutation = useMutation({
        mutationFn: async () => {
            if (isInWishlist) {
                return await removeProductFromWishlist(product._id, userToken);
            } else {
                return await addProductToWishlist(product._id, userToken);
            }
        },
        onSuccess: () => {
            // Invalidate the wishlist query to refetch the updated data
