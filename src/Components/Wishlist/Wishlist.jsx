...
export default function WishList() {
  ...
  // ⚡ Optimization: Use React Query to fetch and manage wishlist state.
  // This ensures the data is always in sync with other components using the 'wishlist' query key.
          ...
          <div className="flex flex-col justify-start items-start">
            ...
            <div className="grid md:grid-cols-4 grid-cols-1 gap-3 rounded-lg ">
              {wishlist.map((product) => (
                <div key={product._id || product.id} className="flex flex-col shadow-xl p-3">
                  <div className="relative">
                    ...
                    <button
                      ...
                      onClick={() => productRemove(product._id || product.id)}
                  ...
                  <div className="flex flex-col justify-start items-start mt-0">
                    <div className="mt-1">
                      <p className="tracking-tight text-base font-medium leading-4 text-gray-600 dark:text-white line-clamp-2">{product.description}</p>
                    </div>
                    <div className="mt-1">
                      <p className="flex justify-start tracking-tight font-bold text-lg leading-4 text-gray-600 dark:text-white mt-2">Price : ${product.price}</p>
                    </div>
                    <div className="flex justify-between flex-col lg:flex-row items-center mt-4 space-y-1 lg:space-y-0 lg:space-x-1 xl:space-x-8 w-full">
                      ...
                      <div className="w-full">
                        <button
                          className="rounded-lg p-3 focus:outline-none focus:ring-green-600 focus:ring-offset-2 focus:ring-2 text-white w-full tracking-tight py-2 text-lg leading-4 hover:bg-green-700 bg-green-600 border border-green-600 dark:bg-white dark:text-green-900 dark:hover:bg-green-700 dark:hover:text-white"
