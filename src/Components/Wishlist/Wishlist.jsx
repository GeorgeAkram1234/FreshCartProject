...
export default function WishList() {
  ...
  // Use React Query to manage the wishlist cache, shared with the Product component
  ...
  async function productRemove(productId) {
    try {
      ...
      // Invalidate the cache to ensure all components stay in sync
      queryClient.invalidateQueries({ queryKey: ['wishlist', userToken] });
    } catch (error) {
      // Error handling is handled in removeProductFromWishlist service
    }
  }
              ...
              <div key={product.id || product._id} className="flex flex-col shadow-xl p-3">
                ...
                <div className="mt-1 flex justify-between items-center ">
                  <div className="flex justify-center items-center">
                    <p className="tracking-tight text-2xl font-semibold leading-6 text-gray-600 dark:text-white">{product.title}</p>
                  </div>

                </div>
                <div className="flex flex-col justify-start items-start mt-0">
                  {/* Preserving product metadata fields for UI consistency */}
                  <div>
                  ...
                  <div className="mt-1">
                    <p className="tracking-tight text-base font-medium leading-4 text-gray-600 dark:text-white">{product.title}</p>
                  </div>
                  <div className="mt-1">
                  ...
                  <div className="flex justify-between flex-col lg:flex-row items-center mt-4  space-y-1 lg:space-y-0 lg:space-x-1 xl:space-x-8">
                    ...
                    <div className="w-full">
                    <button className=" rounded-lg p-3 focus:outline-none focus:ring-green-600 focus:ring-offset-2 focus:ring-2 text-white w-full tracking-tight py-4 text-lg leading-4 hover:bg-green-700 bg-green-600 border border-green-600 dark:bg-white dark:text-green-900 dark:hover:bg-green-700 dark:hover:text-white"
