...
export default function WishList() {
  ...
  async function productRemove(productId) {
    try {
      ...
      // Invalidate the query to refetch the updated wishlist
      queryClient.invalidateQueries({ queryKey: ['wishlist', userToken] });
    } catch (error) {
      // Error handling is already done in removeProductFromWishlist
    }
  }
            ...
            {wishlist.map(product => (
              <div key={product._id} className="flex flex-col shadow-xl p-3">
                ...
                <div className="flex flex-col justify-start items-start mt-0">
                  ...
                  <div className="mt-1">
                    <p className="tracking-tight text-base font-medium leading-4 text-gray-600 dark:text-white">{product.brand?.name}</p>
                  </div>
                  <div className="mt-1">
